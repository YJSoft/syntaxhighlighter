...
function insertCode()
{
	...
	var style = "border:#666 1px dotted;border-left:#2AE 5px solid;padding:5px;background:#FAFAFA url('./modules/editor/components/syntaxhighlighter/code.png') no-repeat top right;";
	var html = '<div editor_component="syntaxhighlighter" code_type="'+opt.code_type+'" title="'+opt.title+'" first_line="'+opt.first_line+'" collapse="'+opt.collapse+'" highlight="'+opt.highlight+'" nogutter="'+opt.nogutter+'" style="'+style+'">'+opt.code+'</div><br /><br />';
...
//줄 표시 비활성화시 시작 줄 번호 속성 편집 불가능하도록 수정
jQuery(document).ready(function($) {
    $('#nogutter').change(function() {
        $('#first_line').prop('disabled', $(this).is(":checked"));
    });
});